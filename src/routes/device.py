...
def get_blueprint():
    ...
    bp.add_url_rule('/device', view_func=device_view, methods=['POST'])
    bp.add_url_rule('/device/<int:device_id>', view_func=device_view, methods=['GET', 'PUT'])

    command_view = JsonEndpoint.as_view('device_command', DeviceCommandController)
    bp.add_url_rule('/device/<int:device_id>/command', view_func=command_view, methods=['POST'])
